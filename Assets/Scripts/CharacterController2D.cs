...
public class CharacterController2D : MonoBehaviour
{
	...
	[Range( 0.8f, 0.999f )]
	public float triggerHelperBoxColliderScale = 0.90f;
	...
	private void moveHorizontally( ref Vector3 deltaMovement )
	{
		...
		for( var i = 0; i < totalHorizontalRays; i++ )
		{
			...
			_raycastHit = Physics2D.Raycast( ray, rayDirection, rayDistance, platformMask & ~oneWayPlatformMask & ~bulletMask);
			if( _raycastHit )
			{
	...
	private void moveVertically( ref Vector3 deltaMovement )
	{
		...
		for( var i = 0; i < totalVerticalRays; i++ )
		{
			...
			DrawRay( ray, rayDirection * rayDistance, Color.red );
			_raycastHit = Physics2D.Raycast( ray, rayDirection, rayDistance, mask & ~bulletMask);
			if( _raycastHit )
			{
				// set our new deltaMovement and recalculate the rayDistance taking it into account
